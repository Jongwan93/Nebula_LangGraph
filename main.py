# main.py
"""Entry point for the LangGraph stock analysis pipeline."""

import argparse
import os

from dotenv import load_dotenv

from graph import run_pipeline
from pipeline_stages import run_stage_gather, run_stage_analyze, run_stage_rank, run_stage_sheets

load_dotenv()

# S&P 500 ticker list (for testing; use a subset via TICKER_SUBSET_SIZE or run full list)
SP500_TICKERS = [
    "MMM", "AOS", "ABT", "ABBV", "ACN", "ADBE", "AMD", "AES", "AFL", "A",
    "APD", "ABNB", "AKAM", "ALB", "ARE", "ALGN", "ALLE", "LNT", "ALL", "GOOGL",
    "GOOG", "MO", "AMZN", "AMCR", "AEE", "AEP", "AXP", "AIG", "AMT", "AWK",
    "AMP", "AME", "AMGN", "APH", "ADI", "AON", "APA", "APO", "AAPL", "AMAT",
    "APTV", "ACGL", "ADM", "ANET", "AJG", "AIZ", "T", "ATO", "ADSK", "ADP",
    "AZO", "AVB", "AVY", "AXON", "BKR", "BALL", "BAC", "BAX", "BDX", "BRK.B",
    "BBY", "TECH", "BIIB", "BLK", "BX", "XYZ", "BK", "BA", "BKNG", "BSX",
    "BMY", "AVGO", "BR", "BRO", "BF.B", "BLDR", "BG", "BXP", "CHRW", "CDNS",
    "CZR", "CPT", "CPB", "COF", "CAH", "KMX", "CCL", "CARR", "CAT", "CBOE",
    "CBRE", "CDW", "COR", "CNC", "CNP", "CF", "CRL", "SCHW", "CHTR", "CVX",
    "CMG", "CB", "CHD", "CI", "CINF", "CTAS", "CSCO", "C", "CFG", "CLX",
    "CME", "CMS", "KO", "CTSH", "COIN", "CL", "CMCSA", "CAG", "COP", "ED",
    "STZ", "CEG", "COO", "CPRT", "GLW", "CPAY", "CTVA", "CSGP", "COST", "CTRA",
    "CRWD", "CCI", "CSX", "CMI", "CVS", "DHR", "DRI", "DDOG", "DVA", "DAY",
    "DECK", "DE", "DELL", "DAL", "DVN", "DXCM", "FANG", "DLR", "DG", "DLTR",
    "D", "DPZ", "DASH", "DOV", "DOW", "DHI", "DTE", "DUK", "DD", "EMN",
    "ETN", "EBAY", "ECL", "EIX", "EW", "EA", "ELV", "EMR", "ENPH", "ETR",
    "EOG", "EPAM", "EQT", "EFX", "EQIX", "EQR", "ERIE", "ESS", "EL", "EG",
    "EVRG", "ES", "EXC", "EXE", "EXPE", "EXPD", "EXR", "XOM", "FFIV", "FDS",
    "FICO", "FAST", "FRT", "FDX", "FIS", "FITB", "FSLR", "FE", "FI", "F",
    "FTNT", "FTV", "FOXA", "FOX", "BEN", "FCX", "GRMN", "IT", "GE", "GEHC",
    "GEV", "GEN", "GNRC", "GD", "GIS", "GM", "GPC", "GILD", "GPN", "GL", "GDDY",
    "GS", "HAL", "HIG", "HAS", "HCA", "DOC", "HSIC", "HSY", "HPE", "HLT",
    "HOLX", "HD", "HON", "HRL", "HST", "HWM", "HPQ", "HUBB", "HUM", "HBAN",
    "HII", "IBM", "IEX", "IDXX", "ITW", "INCY", "IR", "PODD", "INTC", "ICE",
    "IFF", "IP", "IPG", "INTU", "ISRG", "IVZ", "INVH", "IQV", "IRM", "JBHT",
    "JBL", "JKHY", "J", "JNJ", "JCI", "JPM", "K", "KVUE", "KDP", "KEY",
    "KEYS", "KMB", "KIM", "KMI", "KKR", "KLAC", "KHC", "KR", "LHX", "LH",
    "LRCX", "LW", "LVS", "LDOS", "LEN", "LII", "LLY", "LIN", "LYV", "LKQ",
    "LMT", "L", "LOW", "LULU", "LYB", "MTB", "MPC", "MKTX", "MAR", "MMC",
    "MLM", "MAS", "MA", "MTCH", "MKC", "MCD", "MCK", "MDT", "MRK", "META",
    "MET", "MTD", "MGM", "MCHP", "MU", "MSFT", "MAA", "MRNA", "MHK", "MOH",
    "TAP", "MDLZ", "MPWR", "MNST", "MCO", "MS", "MOS", "MSI", "MSCI", "NDAQ",
    "NTAP", "NFLX", "NEM", "NWSA", "NWS", "NEE", "NKE", "NI", "NDSN", "NSC",
    "NTRS", "NOC", "NCLH", "NRG", "NUE", "NVDA", "NVR", "NXPI", "ORLY", "OXY",
    "ODFL", "OMC", "ON", "OKE", "ORCL", "OTIS", "PCAR", "PKG", "PLTR", "PANW",
    "PSKY", "PH", "PAYX", "PAYC", "PYPL", "PNR", "PEP", "PFE", "PCG", "PM",
    "PSX", "PNW", "PNC", "POOL", "PPG", "PPL", "PFG", "PG", "PGR", "PLD",
    "PRU", "PEG", "PTC", "PSA", "PHM", "PWR", "QCOM", "DGX", "RL", "RJF",
    "RTX", "O", "REG", "REGN", "RF", "RSG", "RMD", "RVTY", "ROK", "ROL",
    "ROP", "ROST", "RCL", "SPGI", "CRM", "SBAC", "SLB", "STX", "SRE", "NOW",
    "SHW", "SPG", "SWKS", "SJM", "SW", "SNA", "SOLV", "SO", "LUV", "SWK",
    "SBUX", "STT", "STLD", "STE", "SYK", "SMCI", "SYF", "SNPS", "SYY", "TMUS",
    "TROW", "TTWO", "TPR", "TRGP", "TGT", "TEL", "TDY", "TER", "TSLA", "TXN",
    "TPL", "TXT", "TMO", "TJX", "TKO", "TTD", "TSCO", "TT", "TDG", "TRV",
    "TRMB", "TFC", "TYL", "TSN", "USB", "UBER", "UDR", "ULTA", "UNP", "UAL",
    "UPS", "URI", "UNH", "UHS", "VLO", "VTR", "VLTO", "VRSN", "VRSK", "VZ",
    "VRTX", "VTRS", "VICI", "V", "VST", "VMC", "WRB", "GWW", "WAB", "WBA",
    "WMT", "DIS", "WBD", "WM", "WAT", "WEC", "WFC", "WELL", "WST", "WDC",
    "WY", "WSM", "WMB", "WTW", "WDAY", "WYNN", "XEL", "XYL", "YUM", "ZBRA",
    "ZBH", "ZTS",
]


# Default tickers when not specified (e.g. for --stage gather or full)
DEFAULT_TICKERS = ["NVDA", "TSLA"]


def _parse_args():
    parser = argparse.ArgumentParser(
        description="Run the stock analysis pipeline (full or a single stage).",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument(
        "--stage",
        choices=["gather", "analyze", "rank", "sheets", "full"],
        default="full",
        help="Stage to run: gather (data only), analyze (LLM), rank (top 5), sheets (write), or full (all).",
    )
    parser.add_argument(
        "--tickers",
        type=str,
        default="",
        help="Comma-separated tickers (e.g. NVDA,TSLA,AAPL). Used for gather and full; ignored for analyze/rank/sheets.",
    )
    return parser.parse_args()


def main():
    args = _parse_args()
    sheet_id = os.getenv("GOOGLE_SHEET_ID", "").strip()

    # Resolve ticker list for stages that need it
    if args.tickers:
        tickers = [t.strip().upper() for t in args.tickers.split(",") if t.strip()]
    else:
        tickers = DEFAULT_TICKERS.copy()

    if args.stage == "gather":
        if not tickers:
            print("No tickers to process. Use --tickers NVDA,TSLA,...")
            return
        print("Stage 1: Gather (yfinance + Tavily)", flush=True)
        print("Tickers:", tickers, flush=True)
        run_stage_gather(tickers)
        print("Done.", flush=True)
        return

    if args.stage == "analyze":
        print("Stage 2: Analyze (LLM)", flush=True)
        results = run_stage_analyze()
        print(f"  Analyzed {len(results)} tickers.", flush=True)
        print("Done.", flush=True)
        return

    if args.stage == "rank":
        print("Stage 3: Rank (top 5 positive)", flush=True)
        ranked = run_stage_rank()
        print(f"  Ranked {len(ranked)} results.", flush=True)
        print("Done.", flush=True)
        return

    if args.stage == "sheets":
        if not sheet_id:
            print("GOOGLE_SHEET_ID not set. Set it in .env to write to Sheets.")
            return
        print("Stage 4: Write to Google Sheet", flush=True)
        run_stage_sheets(sheet_id)
        print("Done.", flush=True)
        return

    # Full pipeline (default)
    if not tickers:
        print("No tickers to process. Use --tickers NVDA,TSLA,...")
        return
    print("Running full pipeline for:", len(tickers), "tickers", tickers)
    if sheet_id:
        print("Output sheet:", sheet_id)
    else:
        print("GOOGLE_SHEET_ID not set; results will not be written to Sheets.")
    final_state = run_pipeline(tickers=tickers, sheet_id=sheet_id)
    ranked = final_state.get("ranked_results") or []
    print("\n--- Top performers (written to Sheet) ---")
    for r in ranked:
        print(
            f"{r.get('date')} | {r.get('ticker')} | "
            f"% change: {r.get('predicted_change_pct')} | {r.get('reason', '')[:80]}..."
        )
    if not ranked:
        print("(No positive predictions to rank.)")
    print("Done.")


if __name__ == "__main__":
    main()
